...
namespace ProgrammersBlog.Mvc.Areas.Admin.Controllers
{
    ...
    public class UserController : BaseController
    {
        ...
        public UserController(UserManager<User> userManager, IWebHostEnvironment env, IMapper mapper, SignInManager<User> signInManager, IImageHelper imageHelper, IToastNotification toastNotification) : base(imageHelper, mapper, userManager)
        {
        ...
        public async Task<IActionResult> Add(UserAddDto userAddDto)
        {
            if (ModelState.IsValid)
            {
                ...
                var result = await UserManager.CreateAsync(user, userAddDto.Password); // Password'ü hashlemek için ayrı olarak istiyor onu
                if (result.Succeeded)
                {
        ...
        public async Task<IActionResult> PasswordChange(UserPasswordChangeDto userPasswordChangeDto)
        {
            if (ModelState.IsValid)
            {
                ...
                if (isVerified)
                {
                    ...
                    if (result.Succeeded)
                    {
                        // SecurityStamp değerini değiştireceğiz çünkü bu bize securitystamp değişme tarihinde önemli bir değişiklik olduğu bilgisini veriyor
                ...
                {
                    ModelState.AddModelError("", "Lütfen girmiş olduğunuz şifrenizi kotrol ediniz.");
                    return View(userPasswordChangeDto);
                }
            }
            else
