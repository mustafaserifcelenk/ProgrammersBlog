   ...
   const dataTable = $('#usersTable').DataTable({
                           ...
                           if (userListDto.ResultStatus === 0) {
                               $.each(userListDto.Users.$values,
                                ...
                                <button class="btn btn-danger btn-sm btn-delete" data-id="${user.Id}"><span class="fas fa-minus-circle"></span></button>
                                            `
                                       ]).node(); //node sayesinde add'lediğimiz row'u seçebiliyor oluyoruz
                                       //foreach içindeyken her rowu jquery objesine dönüştürüyoruz, datatable api'dan jquery objesine geçiş yapmış olduk
    ...
    $(function () {
        ...
        $('#btnAdd').click(function () {
            ...
            // get url'den getirdiği veriyi yani partialview'i fonksiyonun içinde data olarak getirir
            $.get(url).done(function (data) {
                placeHolderDiv.html(data);
                //placeholderdiv'in içinde ki .modal classına sahip olan elementleri bul ve onu modal yap ( .modal('show') ile ) ve göster
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        ...
                        if (isValid) {
                            ...
                            dataTable.row(tableRow).invalidate(); // Değişimden table row'u haberdar etme
