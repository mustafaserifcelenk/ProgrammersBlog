...
namespace ProgrammersBlog.Services.Concrete
{
    class CategoryManager : ICategoryService //Category entitimiz ile ilgili işlemleri yönetecek sınıf
    {
        ...
        {
            var categories = await _unitOfWork.Categories.GetAllSync(null, c => c.Articles);
        ...
        {
            var categories = await _unitOfWork.Categories.GetAllSync(c => !c.IsDeleted, c => c.Articles);
            if (categories.Count > -1)
            {
                return new DataResult<CategoryListDto>(ResultStatus.Success, new CategoryListDto
                {
        ...
        {
            var categories = await _unitOfWork.Categories.GetAllSync(c => !c.IsDeleted && c.IsActive, c => c.Articles);
            if (categories.Count > -1)
            {
                return new DataResult<CategoryListDto>(ResultStatus.Success, new CategoryListDto
                {
        ...
        public async Task<IResult> Update(CategoryUpdateDto categoryUpdateDto, string modifiedByName) //update'de add'den farklı olarak id var ve bununla biz güncellenecek nesneyi çağırcaz
        {
